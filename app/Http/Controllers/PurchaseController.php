...
class PurchaseController extends Controller
{
        ...
        public function create($vendorId)
    {
        if (\Auth::user()->isAbleTo('purchase create')) {
            ...
            $product_category = \Workdo\ProductService\Entities\Category::where('created_by', creatorId())
                ->where('workspace_id', getActiveWorkSpace())
                ->where('type', 0)
                ->get()
                ->pluck('name', 'id');

            return view('purchases.create', compact('venders', 'purchase_number', 'product_services', 'category', 'vendorId', 'warehouse', 'customFields', 'product_type', 'projects', 'product_category'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
        ...
        if (\Auth::user()->isAbleTo('purchase create')) {
            if ($request->purchase_type == "product") {
                ...
                for ($i = 0; $i < count($products); $i++) {
                    ...
                    $purchaseProduct->item_category = $products[$i]['item_category'];
